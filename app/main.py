...
async def get_filter_options(db: Session = Depends(get_db)):
    """Filter uchun BARCHA variantlarni olish"""
    try:
        print("🔍 Starting filter options query from trade_data.db...")
        
        # BARCHA davlatlar (trading_partner)
        countries_raw = db.query(distinct(TradeRecord.trading_partner))\
                     .filter(TradeRecord.trading_partner.isnot(None))\
                     .filter(TradeRecord.trading_partner != '')\
                     .filter(TradeRecord.trading_partner != 'NULL')\
                     .order_by(TradeRecord.trading_partner)\
                     .all()
        
        countries = [c[0] for c in countries_raw if c[0] and c[0].strip() and c[0] != 'NULL']
        print(f"🌍 Found {len(countries)} countries from database")

        # BARCHA mahsulotlar (product_name)
        products_raw = db.query(distinct(TradeRecord.product_name))\
                    .filter(TradeRecord.product_name.isnot(None))\
                    .filter(TradeRecord.product_name != '')\
                    .filter(TradeRecord.product_name != 'NULL')\
                    .order_by(TradeRecord.product_name)\
                    .all()
        
        products = [p[0] for p in products_raw if p[0] and p[0].strip() and p[0] != 'NULL']
        print(f"📦 Found {len(products)} products from database")

        # BARCHA yillar
        years_raw = db.query(distinct(TradeRecord.year))\
                 .filter(TradeRecord.year.isnot(None))\
                 .filter(TradeRecord.year > 1990)\
                 .filter(TradeRecord.year <= 2030)\
                 .order_by(TradeRecord.year.desc())\
                 .all()
        
        years = [y[0] for y in years_raw if y[0]]
        print(f"📅 Found {len(years)} years from database")

        # CHEKLOVNI OLIB TASHLASH - barcha ma'lumotlarni qaytarish
        response_data = {
            "success": True,
            "countries": countries,  # Barcha davlatlar
            "products": products,    # Barcha mahsulotlar  
            "years": years          # Barcha yillar
        }
        
        print(f"✅ Returning ALL data: {len(countries)} countries, {len(products)} products, {len(years)} years")
